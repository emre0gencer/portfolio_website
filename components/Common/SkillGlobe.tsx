...
const colToRgba = (col: string, a = 1) => {
  ...
  if (!p) return `255,255,255,${a}`;
  const h = p.h / 360, s = p.s / 100, l = p.l / 100;
...
const brightenHsl = (hsl: string, deltaL: number) => {
  ...
  if (!p) return hsl;
  return `hsl(${p.h} ${p.s}% ${Math.min(100, p.l + deltaL)}%)`;
};
...
export default function SkillGlobe() {
  ...
  const categoryColors: Record<string, string> = Object.fromEntries(
    skills.map((cat, idx) => [cat.category, CATEGORY_COLORS[idx % 5]])
  );
  ...
  useEffect(() => {
    ...
    skills.forEach((cat, catIndex) => {
      ...
      const columnX = marginX + (innerWidth * (catIndex + 0.5)) / numCategories;
      const stepY = catSkills.length === 1 ? 0 : innerHeight / (catSkills.length - 1);
      catSkills.forEach((skill, idx) => {
        const baseY = catSkills.length === 1 ? innerHeight / 2 : stepY * idx;
        nodes.push({
          ...skill,
          x: columnX + (Math.random() - 0.5) * 10,
          y: marginY + baseY + (Math.random() - 0.5) * 6,
          baseSize: 3 + skill.proficiency * 2,
          currentSize: 3 + skill.proficiency * 2,
          currentOpacity: 0.25,
        });
      });
    });
    ...
    skills.forEach((cat, catIndex) => {
      ...
      fromNodes.forEach((source) => {
        const count = Math.min(2, toNodes.length);
        for (let i = 0; i < count; i++) {
          edges.push({ from: source, to: toNodes[Math.floor(Math.random() * toNodes.length)] });
        }
      });
    });

    const findNearest = (cx: number, cy: number): { nearest: Node | null; distance: number } => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const { nearest, distance } = findNearest(canvasX, canvasY);
      if (nearest && distance < 24) {
        hoverSkillNameRef.current = nearest.name;
        setTooltipState({ visible: true, x: localX + 10, y: localY + 10, text: nearest.name });
      } else {
    ...
    const animate = () => {
      ...
      const activeCategory = openCategoryRef.current;

      ctx.globalCompositeOperation = "source-over";
      edges.forEach(({ from, to }) => {
        const isActive = activeCategory && (from.category === activeCategory || to.category === activeCategory);
        const isHovered = hoveredSkillName && (from.name === hoveredSkillName || to.name === hoveredSkillName);
        const strokeCol = isActive ? brightenHsl(categoryColors[from.category] || baseGrey, 5) : baseGrey;
        ctx.strokeStyle = `rgba(${colToRgba(strokeCol, isActive ? (isHovered ? 0.6 : 0.4) : 0.18)})`;
        ctx.lineWidth = isActive ? (isHovered ? 1.4 : 1.0) : 0.7;
        ctx.beginPath();
      ...
      nodes.forEach((node, idx) => {
        ...
        let targetSize = node.baseSize, targetOpacity = 0.22, fillColor = baseGrey;
        if (isActive) { fillColor = categoryColors[node.category]; targetSize = node.baseSize + 2.5; targetOpacity = 0.7; }
        if (isHovered) { fillColor = brightenHsl(categoryColors[node.category], 10); targetSize = node.baseSize + 5; targetOpacity = 1; }
        node.currentSize += (targetSize - node.currentSize) * 0.2;
        node.currentOpacity += (targetOpacity - node.currentOpacity) * 0.2;
        ctx.fillStyle = `rgba(${colToRgba(fillColor, node.currentOpacity)})`;
        ctx.beginPath();
        ctx.arc(node.x, y, Math.max(1.5, node.currentSize), 0, Math.PI * 2);
        ctx.fill();
        if (isHovered || isActive) {
          ctx.strokeStyle = `rgba(${colToRgba(brightenHsl(categoryColors[node.category] || baseGrey, isHovered ? 15 : 5), 0.35)})`;
          ctx.lineWidth = isHovered ? 2 : 1;
          ctx.beginPath();
          ctx.arc(node.x, y, Math.max(1.5, node.currentSize) + (isHovered ? 6 : 4), 0, Math.PI * 2);
          ctx.stroke();
        }
      });
  ...
  const categoryCounts = skills.map((cat) => ({
    category: cat.category,
  ...
  const featuredWork = selectedSkill ? [
    ...projects.filter((p) => p.techStack.includes(selectedSkill)).map((p) => ({
      key: `proj-${p.id}`, title: p.title,
      summary: p.description.split(".")[0] + ".",
      href: `/projects/${p.id}`, type: "Project" as const,
    })),
    ...experiences.filter((e) => e.skills.includes(selectedSkill)).map((e) => ({
      key: `exp-${e.id}`, title: `${e.role} @ ${e.organization}`,
      summary: e.description[0], href: `/experience?view=cards#${e.id}`, type: "Experience" as const,
    })),
    ...courses.filter((c) => c.categories.includes(selectedSkill)).map((c) => ({
      key: `course-${c.id}`, title: c.title,
      summary: c.description ? c.description.split(".")[0] + "." : "",
      href: `/courses#${c.id}`, type: "Course" as const,
    })),
  ] : [];

  const toggleDropdown = (category: string) => {
    setOpenCategory((prev) => { if (prev === category) { setSelectedSkill(null); return null; } return category; });
  };
    ...
    >
      <div className="relative">
        <canvas
          ref={canvasRef}
          data-category-box
          className="w-full h-auto max-w-lg rounded-2xl bg-gradient-to-br from-background via-background/90 to-background shadow-[0_0_60px_rgba(15,23,42,0.9)]"
        />
          ...
          {categoryCounts.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full p-3 rounded-lg border transition-all duration-200 text-center cursor-pointer ${CATEGORY_BORDER[item.category] ?? ""} ${
                  isOpen
              ...
              >
                ...
                <ChevronDown className={`h-3 w-3 mx-auto mt-1 text-primary/60 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
              </button>
            );
          })}
        </div>

        {/* Skill badges dropdown */}
        <div
                ...
                <div className="mt-2 rounded-xl border border-primary/30 bg-background/95 backdrop-blur-sm shadow-[0_8px_32px_rgba(59,130,246,0.2)] p-3">
                  <div className="flex flex-wrap gap-1.5 justify-center">
                    {item.skills.map((skill) => (
                      <button
                        key={skill}
                        onClick={(e) => { e.stopPropagation(); setSelectedSkill((prev) => prev === skill ? null : skill); }}
              ...
              <ul className="space-y-2">
                {featuredWork.map((item) => (
                  <li key={item.key}>
                    ...
                    >
                      <span className={`mt-0.5 shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded ${TYPE_PILL[item.type]}`}>
                        {item.type}
                      </span>
      ...
      <p className="text-center text-sm text-muted-foreground max-w-md">
        Each column is a <span className="font-semibold text-foreground">skill category</span>,
