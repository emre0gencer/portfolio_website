...
export const projects: Project[] = [
  {
    ...
    techStack: ['Ruby on Rails', 'Virtual Machines', 'JavaScript', 'Bytecode Interpreters', 'ActiveRecord', 'Stack & Call Frame Design', 'Performance Benchmarking', 'HTML', 'CSS', 'MVC Architecture', 'Ruby'],
    category: 'Systems',
    categories: ['Systems', 'Web Dev', 'Full-Stack Development'],
    date: '2026',
    githubUrl: 'https://github.com/emre0gencer/executionlabruby',
    details: {
      problem: 'Understanding how VM design trade-offs manifest at runtime is difficult without being able to observe them directly. Stack-based vs. register-based execution models, call frame management, and opcode dispatch strategies are well-defined in theory but hard to compare interactively against real bytecode programs.',
      ...
      contribution: 'Sole developer responsible for VM backend implementations, bytecode program library, Rails MVC architecture, ActiveRecord data modeling, execution trace logging, performance benchmarking pipeline, and frontend visualization of runtime metrics.',
      learned: ['Translating low-level VM concepts from C into idiomatic Ruby', 'Designing multiple interchangeable VM backends under a shared interface', 'Surfacing execution internals (stack state, frame depth, cycle counts) as structured data', 'Rails MVC patterns for data-driven visualization features', 'Benchmarking and comparing runtime behavior across implementation strategies'],
    },
  },
  {
    id: 'applyeasy',
    ...
    details: {
      problem: 'Tailoring a resume to each job posting is time-consuming and error-prone — most applicants either submit generic resumes or over-optimize with keyword stuffing, both of which reduce their chances with modern ATS systems and human reviewers.',
      ...
      contribution: 'Sole full-stack developer responsible for the FastAPI backend pipeline, React/TypeScript frontend, SQLite data layer, LLM prompt engineering and integration, semantic matching logic, Jinja2-based PDF templating, and end-to-end validation system.',
      learned: ['Designing multi-step LLM pipelines with validation and guardrails', 'Semantic similarity scoring for job-resume fit analysis', 'FastAPI backend architecture and API design', 'Prompt engineering for structured, factual LLM output', 'PDF generation from server-side templates with Jinja2', 'Full-stack integration across Python backend and React frontend'],
    },
  },
  {
    ...
    date: '2026',
    details: {
      problem: 'Design and build a reliable store management system from scratch, beginning with a well-structured data model and implementing the full application layer — from database constraints to user-facing views — within the conventions of a full-stack framework.',
      solution: 'Designed an ERD to model the domain, then translated it into Rails ActiveRecord models with validations, associations, and database-level constraints. Implemented the full MVC stack: RESTful routes, controllers handling business logic, and views surfacing functionality to users. Prioritized correctness and maintainability throughout the data workflows.',
      contribution: 'Sole developer responsible for ERD design, schema implementation, ActiveRecord modeling, route and controller development, view construction, and end-to-end testing of relational data workflows.',
      learned: ['ERD design and schema-to-code translation in Rails', 'ActiveRecord associations, validations, and database constraints', 'Full-stack MVC development with Rails conventions', 'Backend integration and database-backed feature design', 'Component-based thinking applied to server-rendered web applications'],
    },
  },
  {
    id: 'c0vm',
    ...
    techStack: ['C', 'Systems Programming', 'Virtual Machines', 'Bytecode Interpreters', 'Memory Management', 'Stack & Call Frame Design', 'Low-Level Debugging', 'Valgrind'],
    category: 'Systems',
    categories: ['Systems', 'Course Project', 'Low-Level Programming'],
    date: '2025',
    details: {
      problem: 'Design and implement a complete virtual machine capable of executing arbitrary C0 programs from compiled bytecode, while ensuring correct control flow, memory safety, stack discipline, and precise runtime error reporting — all in an unsafe language (C).',
      solution: 'Implemented a stack-based abstract machine inspired by the JVM and LLVM, including an operand stack, program counter, call stack with activation records, local variable environments, constant pools, and a dynamically allocated heap for arrays and structs. The VM decodes and executes dozens of bytecode instructions, handles function invocation and returns, and enforces runtime safety checks required by the C0 language specification.',
      contribution: 'Individually implemented the VM execution engine, instruction dispatch loop, arithmetic and control-flow opcodes, call stack and frame management, heap allocation for arrays and structs, memory access instructions, and runtime error handling. Wrote extensive manual tests, debug traces, and used Valgrind to verify correctness and acceptable memory behavior.',
      learned: ['Virtual machine and interpreter architecture', 'Call stack and activation record management', 'Low-level memory modeling for arrays and structs', 'Runtime safety checks and error signaling', 'Bridging compiler output (bytecode) with execution semantics', 'Debugging complex stateful systems in C'],
    },
  },
  {
    ...
    techStack: ['PostgreSQL', 'SQL (DDL & DML)', 'BCNF Normalization', 'Triggers & Trigger Functions', 'Window Functions', 'Python', 'psycopg2', 'CSV-based Data Loading', 'Vertabelo'],
    category: 'Databases',
    categories: ['Databases', 'Course Project', 'Backend Systems'],
    date: '2025',
    githubUrl: 'https://github.com/emre0gencer/instacartdb',
    details: {
      problem: 'Design a scalable database system that supports diverse real-world user roles (customers, shoppers, retailers) and complex behaviors such as analytics-heavy budget tracking, earnings computation, order monitoring, and multi-store cart management — while maintaining strict data integrity and normalization guarantees.',
      solution: 'Developed an Instacart-inspired relational database by incrementally converting user stories into a conceptual model, relational schema, and BCNF-normalized design. Implemented the physical database in PostgreSQL with carefully chosen primary and foreign keys, enforced integrity via triggers, and supported analytical workloads using window functions. Each user story was executed through Python programs that interact directly with the database and demonstrate before/after state transitions.',
      contribution: 'Co-designed the conceptual and relational models, performed functional dependency analysis and BCNF normalization, implemented PostgreSQL tables and constraints via generated DDL, authored SQL triggers and analytical queries, and built Python scripts using psycopg2 to execute and demonstrate all 10 user stories with traceable output.',
      learned: ['End-to-end database design lifecycle (conceptual → logical → physical)', 'Functional dependencies and BCNF normalization in practice', 'Designing triggers for automatic state maintenance', 'Using window functions for analytical queries', 'Bridging SQL systems with Python application logic', 'Incremental system growth driven by user stories'],
    },
  },
  {
    id: 'duquesne-incline',
    ...
    details: {
      ...
      learned: ['API integration with third-party services', 'Responsive web design principles', 'jQuery for DOM manipulation and effects', 'User experience design for visitor information'],
    },
  },
  {
    ...
    techStack: ["TypeScript", "React", "HTML", "CSS", "Tailwind CSS", "Vite", "React Router", "shadcn/ui", "GitHub Pages", "GitHub Actions"],
    category: 'Web Dev',
    ...
    details: {
      problem: 'Create a professional online presence to showcase technical skills, projects, and experience in an organized and visually appealing way that reflects modern web development practices.',
      solution: 'Developed a full-featured portfolio website using React and TypeScript with modern UI components from shadcn/ui. Implemented responsive design with Tailwind CSS, dynamic routing for project details, filtering systems for projects and experiences, and smooth scroll animations for enhanced user experience.',
      contribution: 'Sole developer responsible for all aspects: component architecture, TypeScript interfaces for data models, responsive layout design, deployment configuration, and continuous integration via GitHub Actions.',
      learned: ['React component composition and prop patterns', 'TypeScript for type-safe React development', 'Tailwind CSS utility-first styling', 'Vite build configuration and optimization', 'GitHub Pages deployment workflow', 'UI/UX design principles for portfolio sites'],
    },
  },
  {
    id: 'basketball-duels',
    ...
    details: {
      ...
      learned: ['Game development with pygame', 'AI difficulty balancing through cooldown mechanics', 'State machine architecture for game modes', 'Rapid prototyping and delivery under time constraints'],
    },
  },
  {
    id: 'tok-tut',
    ...
    details: {
      ...
      learned: ['Large-scale data processing and visualization', 'Crisis response technology development', 'Community impact through technology', 'Team coordination in high-pressure situations'],
    },
  },
  {
    id: 'visiosoft',
    ...
    details: {
      ...
      learned: ['Transfer Learning applications in security', 'Real-time video processing at scale', 'Ethical considerations in AI surveillance', 'Human-centered AI design principles'],
    },
  },
  {
    ...
    details: {
      ...
      learned: ['Chrome Extension development', 'API integration and data handling', 'Cross-platform web development', 'User experience optimization'],
    },
  },
  {
    id: 'hydroport',
    ...
    details: {
      ...
      learned: ['IoT systems integration', 'Satellite data processing', 'Sustainable technology design', 'Social impact through innovation'],
    },
  },
];
